...
def dataCleaner(df):
    ...
    print("Continent invalid for", before-len(df), "rows")
    before=len(df)
    ...
    print("Country Code/Indicator Name/Indicator Code invalid for", before-len(df), "rows")
    before=len(df)
    ...
    print("All GDP years empty for", before-len(df), "rows")
    before=len(df)
